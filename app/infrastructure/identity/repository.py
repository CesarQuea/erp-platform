...
from app.infrastructure.identity.models import (
    AuthSessionRow,
    CompanyAccessRow,
...
class SqlAlchemyIdentityRepository:
    ...
    def add_user(self, user: UserAccount) -> None:
        self._session().add(
            UserAccountRow(
                id=user.id,
                login=user.login,
                login_normalized=user.login_normalized,
    ...
    def get_user_by_normalized_login(self, login_normalized: str) -> UserAccount | None:
        row = self._session().execute(
            select(UserAccountRow).where(
                UserAccountRow.login_normalized == login_normalized
            )
        ).scalar_one_or_none()
        return self._user(row) if row else None
    ...
    def set_user_status(
        ...
        self._session().execute(
            ...
            .where(UserAccountRow.id == user_id)
    ...
    def revoke_refresh_for_user(self, user_id: UUID, at: datetime) -> None:
        session_ids = select(AuthSessionRow.id).where(AuthSessionRow.user_id == user_id)
        self._session().execute(
            update(RefreshTokenRow)
            .where(
                RefreshTokenRow.session_id.in_(session_ids),
                RefreshTokenRow.revoked_at.is_(None),
            )
            .values(revoked_at=at)
        )
    ...
    def list_memberships(self, user_id: UUID) -> Sequence[TenantMembership]:
        rows = self._session().execute(
            select(TenantMembershipRow).where(TenantMembershipRow.user_id == user_id)
        ).scalars().all()
    ...
    def set_membership_status(
        ...
        self._session().execute(
            ...
            .where(TenantMembershipRow.id == membership_id)
            .values(status=status.value, updated_at=updated_at)
        )
    ...
    def list_company_access(self, membership_id: UUID) -> Sequence[CompanyAccess]:
        rows = self._session().execute(
            select(CompanyAccessRow).where(
                CompanyAccessRow.membership_id == membership_id
            )
        ).scalars().all()
    ...
    def set_company_access_status(
        ...
        self._session().execute(
            update(CompanyAccessRow)
            .where(
                CompanyAccessRow.membership_id == membership_id,
                CompanyAccessRow.company_id == company_id,
            )
            .values(status=status.value, updated_at=updated_at)
        )
    ...
    def get_role_by_code(self, code: str) -> Role | None:
        row = self._session().execute(
            select(RoleRow).where(RoleRow.code == code)
        ).scalar_one_or_none()
        return self._role(row) if row else None

    def grant_permission_to_role(self, role_id: UUID, permission_id: UUID) -> None:
        if self._session().get(RolePermissionRow, (role_id, permission_id)) is None:
            self._session().add(
                RolePermissionRow(role_id=role_id, permission_id=permission_id)
            )
    ...
    def add_role_assignment(self, assignment: RoleAssignment) -> None:
        self._session().add(
            RoleAssignmentRow(
                ...
                scope_key=_scope_key(assignment),
    ...
    def delete_role_assignment(self, assignment_id: UUID) -> None:
        self._session().execute(
            delete(RoleAssignmentRow).where(RoleAssignmentRow.id == assignment_id)
        )

    def list_role_assignments(self, user_id: UUID) -> Sequence[RoleAssignment]:
        rows = self._session().execute(
            select(RoleAssignmentRow).where(RoleAssignmentRow.user_id == user_id)
        ).scalars().all()
        return [
            RoleAssignment(
                id=row.id,
    ...
    def get_roles(self, role_ids: Sequence[UUID]) -> Sequence[Role]:
        if not role_ids:
            return []
        rows = self._session().execute(
            select(RoleRow).where(RoleRow.id.in_(role_ids))
        ).scalars().all()
    ...
    def get_permission_codes_by_roles(
        ...
        if not role_ids:
            return {}
        rows = self._session().execute(
            select(RolePermissionRow.role_id, PermissionRow.code)
            .join(
                PermissionRow,
                PermissionRow.id == RolePermissionRow.permission_id,
            )
